...
- 新增 **Greasy Fork 用户脚本产物** `dist/dynshot.user.js`：由 `userscript/installer.js` 模板内嵌组件代码生成，
...
- 新增 **archify 交互式架构图** `docs/architecture.html`（源规范 `docs/archify/dynshot.architecture.json`），
...
- 迁移前的 dynshot 独立插件形态：SnapDOM 元素级截图、多图重排、底部留白、懒加载与媒体暂停，同时提供油猴脚本与 Chrome 扩展两种分发方式。
